...
//var i = 0 is start point
//i<100 is conditional (how many times to we want the loop to run)
//i++ is the same as saying i=i+1
for(var i = 0; i<snowAmount; i++){
  snowX[i] = random(0,width); //random number is from 0 to the width - 1, or 0 to 399
  ...
  if(seasonType == "fall"){
    ...
    for(var f = 0; f<leafAmount; f++){
      if(leafY[f]<height-20){
        leafY[f]++;
        leafX[f] = leafX[f] + sin(radians(frameCount)); //leaf sway
